我为你的 `skill-localize` 仓库创建了一个专业的 `README.md`，内容清晰、结构友好，符合 GitHub 开源项目的常见风格。你可以将下面的内容保存为 `README.md` 文件，并上传到仓库根目录。
...
- 确保文件保存为 **UTF-8** 编码
- YAML 格式中冒号后必须有一个空格
- 描述中包含特殊字符时请使用引号包裹（如冒号、引号本身）
- **严禁修改 `name` 和 `triggers` 字段**，否则可能导致 Skill 无法正常触发
